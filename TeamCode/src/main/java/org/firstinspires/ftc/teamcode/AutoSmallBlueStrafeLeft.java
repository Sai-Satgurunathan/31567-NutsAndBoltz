...
@com.qualcomm.robotcore.eventloop.opmode.Autonomous(name="AutoSmallBlueStrafeLeft", group="Linear Opmode")
public class AutoSmallBlueStrafeLeft extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        // ==============================
        // 1️⃣ Move forward 50 inches
        // ==============================
        moveInches(90, 0.5, MoveDirection.FORWARD);
        turnDegrees(75, 0.35, TurnDirection.LEFT);
        moveInches(12, 0.5, MoveDirection.RIGHT);
       // turnDegrees(560, 0.35, TurnDirection.LEFT);


        // 1️⃣ Turn Left 45 degrees
...
//added  code to reduce the power on first shot
    ...
    private void turnDegrees(double degrees, double power, TurnDirection direction) {

        // Tune these for your robot:
        final int countsPerRev = 537;          // your drive motor encoder counts per rev
        final double wheelDiameter = 4.0;      // inches
        final double trackWidth = 15.0;        // inches: center-left wheel to center-right wheel
        final double turnK = 1.00;             // calibration factor (adjust until angle is correct)

        double countsPerInch = countsPerRev / (Math.PI * wheelDiameter);

        // In-place turn arc length per side
        double arcInches = Math.PI * trackWidth * (degrees / 360.0);
        int counts = (int) Math.round(arcInches * countsPerInch * turnK);

        int sign = (direction == TurnDirection.LEFT) ? -1 : 1;
        ...
        double timeoutSec = 2.0; // adjust if needed

        while (opModeIsActive()
                && (frontLeft.isBusy() || frontRight.isBusy() || backLeft.isBusy() || backRight.isBusy())
                && t.seconds() < timeoutSec) {
            idle();
        }
